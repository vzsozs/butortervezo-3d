...
// --- TAB KEZELÉS ---
const activeTab = ref<'furniture' | 'room'>('furniture');
...
<template>
  <div @mousedown.stop
    ...
    <!-- 1. HEADER (KÖZÖS) -->
    <div class="flex-shrink-0 p-6 border-b border-gray-800 bg-gradient-to-b from-gray-800/50 to-transparent">
      <h1 class="text-2xl font-bold text-white tracking-tight">Tervező</h1>
      <p class="text-xs text-blue-400 font-mono mt-1">{{ appVersion }}</p>
    </div>

    <!-- 2. TAB NAVIGÁCIÓ (ÚJ) -->
    <div class="flex-shrink-0 px-4 pb-4 border-b border-gray-800 bg-gradient-to-b from-gray-800/50 to-transparent">
      <div class="flex p-1 bg-black/40 rounded-lg border border-gray-700/50 backdrop-blur-sm">

        <!-- BÚTOROK GOMB -->
        <button @click="activeTab = 'furniture'"
          class="flex-1 py-2 text-xs font-semibold  tracking-wider rounded-md transition-all duration-200" :class="activeTab === 'furniture'
            ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/20 transform scale-105'
        ...
        <button @click="activeTab = 'room'"
          class="flex-1 py-2 text-xs font-semibold  tracking-wider rounded-md transition-all duration-200" :class="activeTab === 'room'
            ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/20 transform scale-105'
            : 'text-gray-500 hover:text-gray-300 hover:bg-white/5'">
          Helyiség
        </button>
    ...
    <div class="flex-1 min-h-0 flex flex-col overflow-hidden relative">
      ...
      <!-- A) BÚTORTERVEZŐ TAB -->
      <div v-if="activeTab === 'furniture'" class="flex flex-col h-full">

        <!-- GLOBÁLIS BEÁLLÍTÁSOK -->
        <div class="flex-shrink-0 p-4 space-y-2 overflow-y-auto custom-scrollbar" style="max-height: 50vh;">
          ...
          <div v-for="group in configStore.globalGroups" :key="group.id"
            class="space-y-2 pb-2 border-b border-gray-800 last:border-0">
              ...
              <div v-else-if="group.construction?.enabled && group.targets.includes('worktops')">
                <div class="relative group">
                  <select v-model="proceduralStore.worktop.thickness"
                    class="w-full bg-[#2a2a2a] border border-gray-700 text-gray-200 text-xs rounded-md py-2 pl-2 pr-4 appearance-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors cursor-pointer hover:bg-[#333]">
                    <option v-for="thickness in group.construction.thicknessOptions" :key="thickness"
                      :value="thickness">
            ...
            <div v-if="group.construction?.enabled && isStandardLegSelected(group)"
              ...
              <div class="flex justify-between mb-1">
                ...
                <span class="text-xs text-blue-400 font-mono">{{ (proceduralStore.plinth.height * 100).toFixed(1) }}
                  cm</span>
              </div>
              <input type="range" :min="group.construction.minHeight || 0.05"
                :max="group.construction.maxHeight || 0.20" step="0.005" v-model="proceduralStore.plinth.height"
        ...
        <div class="flex-grow border-t border-gray-800 bg-[#1a1a1a] flex flex-col overflow-hidden">
          ...
          <div class="flex-grow overflow-y-auto p-4 space-y-6 custom-scrollbar">
            <div v-for="category in configStore.furnitureCategories" :key="category.name">
              <h3 class="text-sm font-semibold text-gray-300 mb-3 pl-1 border-l-2 border-blue-500">{{ category.name }}
              </h3>
                ...
                <div v-for="furniture in category.items" :key="furniture.id">
                  <button @click="settingsStore.setActiveFurnitureId(furniture.id)"
                    ...
                    <div class="flex flex-col items-start">
                      ...
                      <span class="text-xs text-gray-500">{{ (furniture as any).dimensions?.width || '?' }}cm
                        széles</span>
                    </div>
                    <div
      ...
      <!-- B) HELYISÉGTERVEZŐ TAB -->
      <div v-else-if="activeTab === 'room'" class="flex flex-col h-full p-4 overflow-hidden">
        <RoomEditor />
      </div>
