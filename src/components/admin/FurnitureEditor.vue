...
const groupedDisplay = computed(() => {
  ...
  Object.keys(slotTemplates).forEach(key => {
    const template = slotTemplates[key];
    if (template) {
      groups[key] = {
        title: template.name,
        slots: []
  ...
  editableFurniture.value.componentSlots.forEach(slot => {
    ...
    for (const [key, template] of Object.entries(slotTemplates)) {
      // FELTÉTEL 1: Típus egyezés (Normál működés)
      const typeMatch = slot.componentType === template.type;

      // FELTÉTEL 2 (JAVÍTÁS): ID alapú kényszerítés a Korpuszhoz
      // Ha a csoport a 'corpus', és a slot ID-ja 'corpus'-szal kezdődik, akkor fogadjuk el.
...
// Ellenőrzi, hogy van-e már létrehozva bármilyen séma
...
function isGroupManagedByLayout(groupKey: string): boolean {
  ...
  // Pl. ha groupKey='front', keresünk type='front' sémát.
  // Ha groupKey='handle', az is a 'front' sémához tartozik.
...
function recompileLayout() {
  ...
  // JAVÍTÁS 1: Nem a Komponens ID kell, hanem a Slot ID!
  // A getRootComponentId() a definíciót adja vissza, nekünk a slot kell.
  const existingSlots = editableFurniture.value.componentSlots || [];
  const oldRootSlot = existingSlots.find(s => s.slotId.startsWith('corpus'));

  // Ha nincs korpusz slot, nincs mit tenni (de ez nem fordulhat elő elvileg)
  if (!oldRootSlot) return;

  const rootComponentId = oldRootSlot.defaultComponent; // Ez a komponens ID (pl. 'also_szekreny_60')

  if (schema && rootComponentId) {
    ...
    // 2. Generáljuk le az ÚJ slotokat
    const generatedSlots = compiler.compile(rootComponentId, schema.apply, existingSlots, schema.id, schema.slotProperties);

    // 3. KORPUSZ JAVÍTÁS: Keressük meg az új korpusz slotot (ugyanazzal az ID-val, mint a régi)
    const newRootSlot = generatedSlots.find(s => s.slotId === oldRootSlot.slotId);

    if (newRootSlot) {
      // JAVÍTÁS 2: Most már biztosan megtaláljuk, így átmásoljuk az adatokat
      console.log('Restoring Corpus settings...');
      newRootSlot.defaultComponent = oldRootSlot.defaultComponent;
      newRootSlot.allowedComponents = oldRootSlot.allowedComponents; // <-- EZ VOLT A HIÁNYZÓ LÁNCSZEM!
      newRootSlot.componentType = oldRootSlot.componentType;
      newRootSlot.name = oldRootSlot.name;

      if (oldRootSlot.properties) newRootSlot.properties = oldRootSlot.properties;
    } else {
      console.warn('Critical: Could not find new root slot to restore settings.');
    }

    // 4. LÁBAK (MANUÁLIS ELEMEK) MENTÉSE:
    const manualSlots = existingSlots.filter(s =>
      !s.isAutoGenerated &&
      s.slotId !== oldRootSlot.slotId // A korpuszt már kezeltük
    );

    const validManualSlots = manualSlots.filter(manualSlot => {
      if (manualSlot.attachToSlot === oldRootSlot.slotId) return true;
      return generatedSlots.some(gen => gen.slotId === manualSlot.attachToSlot);
    });

    // 5. Összefésülés
    editableFurniture.value.componentSlots = [...generatedSlots, ...validManualSlots];
  }
}
...
<template>
  <div class="admin-panel h-full flex flex-col" v-if="editableFurniture">
    ...
    <div v-show="activeTab === 'general'" class="flex-1 overflow-y-auto space-y-6 pb-10">
      ...
      <!-- ÚJ ELEM HOZZÁADÁS (Csak a nem layoutos elemekhez engedjük?) -->
      <div class="mb-6">
      ...
      <!-- DINAMIKUS LISTA (VÉGLEGES LOGIKA) -->
      ...
      <!-- ÁRVÁK (Ha van valami, ami nem illik a fenti kategóriákba) -->
