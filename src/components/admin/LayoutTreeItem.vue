...
function updateSlotProperty(pointId: string, key: keyof ComponentSlotConfig, value: any) {
    const slot = props.getSlot(props.path, pointId);
    if (slot) {
        emit('update:slot', slot.slotId, { [key]: value });
    }
}
        ...
        <div v-if="attachmentPoints.length > 0" class="pl-4 border-l-2 border-gray-100 ml-2">
            <div v-for="point in attachmentPoints" :key="point.id" class="mb-3">
                ...
                <div class="flex items-center gap-2">
                    <select :value="getAssignedComponentId(point.id) || ''"
                        @change="handleComponentChange(point.id, $event)"
                        ...
                        <option v-for="opt in getAvailableComponents(point.allowedComponentTypes)" :key="opt.id"
                            :value="opt.id">
                            {{ opt.name }}
                        </option>
                    </select>
