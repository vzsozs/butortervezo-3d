...
const standaloneSlots = computed(() => {
  ...
  // KIVETTÜK A SZŰRÉST: Mostantól minden slot látszik a Részletekben is,
  // így lehet cserélni az ajtót akkor is, ha csoport része.
  return allSlots
})
...
function getTargetWidth(): number | undefined {
  const object = selectionStore.selectedObject
  if (!object || !object.userData.componentState) return undefined

  // 1. Keressük meg a "corpus" slotot vagy bármit, aminek a nevében benne van, hogy "corpus"
  const componentState = object.userData.componentState
  const config = object.userData.config

  // Keressük a slot definíciót
  const corpusSlot = config?.componentSlots.find((s: any) => s.slotId.includes('corpus'))

  if (corpusSlot) {
    const componentId = componentState[corpusSlot.slotId]
    const componentDef = configStore.getComponentById(componentId)
    return componentDef?.width
  }

  return undefined
}

function getFilteredComponents(slot: any) {
  const targetWidth = getTargetWidth()
  console.log(`[InspectorPanel] Smart Filter - Target Width: ${targetWidth}, Slot: ${slot.slotId}`)

  // Ha nincs szélesség definíció a korpuszon, minden mehet
  if (targetWidth === undefined) return slot.allowedComponents

  return slot.allowedComponents.filter((compId: string) => {
    const comp = configStore.getComponentById(compId)
    // Ha a komponensnek nincs szélessége, akkor univerzális -> mehet
    if (!comp || comp.width === undefined) {
      console.log(`  -> Component ${compId} allowed (universal)`)
      return true
    }

    // Ha van szélessége, egyeznie kell
    const match = Math.abs(comp.width - targetWidth) < 0.1
    console.log(`  -> Component ${compId} width: ${comp.width} vs target: ${targetWidth} => ${match ? 'ALLOWED' : 'HIDDEN'}`)
    return match
  })
}
...
<template>
  <div v-if="selectionStore.selectedObject && selectionStore.selectedObjectConfig" ref="panelRef" :style="style"
    ...
    <div class="p-4 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
      ...
      <div v-if="standaloneSlots.length > 0" class="space-y-4">
        ...
        <div v-for="slot in standaloneSlots" :key="slot.slotId"
          class="bg-[#252525] p-3 rounded border border-gray-700/50">
          ...
          <div v-if="slot.allowedComponents.length > 1" class="mb-2 relative">
            <select :value="getCurrentComponentId(slot.slotId)"
              ...
              <option v-for="compId in getFilteredComponents(slot)" :key="compId" :value="compId">
          ...
          <div class="relative">
            ...
            <div class="absolute left-2 top-1/2 transform -translate-y-1/2 w-4 h-4 rounded-full border border-gray-500"
      ...
      <div class="pt-4 grid grid-cols-2 gap-3 border-t border-gray-700">
        <button @click="handleDuplicate"
          class="flex items-center justify-center px-4 py-2 bg-gray-700 text-blue-400 rounded hover:bg-gray-600 transition-colors text-sm font-medium">Duplikálás</button>
        <button @click="handleDelete"
          class="flex items-center justify-center px-4 py-2 bg-gray-700 text-red-400 rounded hover:bg-gray-600 transition-colors text-sm font-medium">Törlés</button>
      </div>
    </div>
  </div>
</template>
