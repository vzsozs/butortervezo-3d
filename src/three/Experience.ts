...
export default class Experience {
  ...
  public async updateGlobalStyles() {
    if (this.isUpdating) {
      console.warn('--- [updateGlobalStyles] SKIPPED (Already running) ---')
      return
    }

    this.isUpdating = true
    // Abort previous operations if any
    if (this.abortController) {
      this.abortController.abort('New updateGlobalStyles call')
    }
    this.abortController = new AbortController()
    const signal = this.abortController.signal

    try {
      // 1. Jelenlegi állapot mentése
      const currentObjects = this.experienceStore.placedObjects
      const globalStyles = this.settingsStore.globalStyleSettings
      const cleanState: SceneState = currentObjects.map((obj) => {
        ...
        if (config) {
          for (const slot of config.componentSlots) {
            ...
            for (const [targetSlotType, targetFamilyId] of Object.entries(globalStyles)) {
              const isMatch =
                slotId.includes(targetSlotType) || slot.componentType === targetSlotType
      ...
      await this.loadState(cleanState, signal) // Épít tiszta lappal
    } catch (error: any) {
      if (error.name === 'AbortError') {
        console.warn('--- [updateGlobalStyles] Aborted:', error.message)
      } else {
        console.error('--- [updateGlobalStyles] ERROR:', error)
      }
    } finally {
      this.isUpdating = false
      this.abortController = null
      console.log('--- [updateGlobalStyles] Kész (UNLOCKED) ---')
    }
  }
